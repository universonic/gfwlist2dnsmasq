...
# Version:     0.6.4 (2017.07.15)
# Author:      Cokebar Chi, Alfred Chou
# Website:     https://github.com/universonic/gfwlist2dnsmasq
...
get_args(){
    ...
    if [ -z $GFWLIST_USERFILE ]; then
        printf '\033[31mError: Please specify the path to the local gfwlist file (with -g/--gfwlist-file [ARGUMENT]).\033[m\n'
        exit 1
    elif [ ! -f $GFWLIST_USERFILE ]; then
        printf '\033[31mError: GFWList must be a regular file.\033[m\n'
        exit 1
    fi

    # Check path & file name
    if [ -z $OUT_FILE ]; then
        printf '\033[31mError: Please specify the path to the output file (with -o/--output [ARGUMENT]).\033[m\n'
        exit 1
    else
        if [ -z ${OUT_FILE##*/} ]; then
...
process(){
    ...
    if [ $? != 0 ]; then
        if [ -z $GFWLIST_USERFILE ]; then
            ...
            printf '\033[33m\nFailed to fetch gfwlist.txt. Will use user-specified gfwlist file...\033[m\n'
            cp $GFWLIST_USERFILE $GFWLIST_FILE
        fi
    fi
